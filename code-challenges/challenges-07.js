...
const classesAvg = (data) => {
    data.grades.forEach((grade) => {
      grade.classes.forEach((classData) => {
        let sum = 0;
        classData.classScores.forEach((score) => {
          sum += score;
        });
        const avg = Math.floor(sum / classData.classScores.length);
        classData.avg = avg;
      });
    });
  };
